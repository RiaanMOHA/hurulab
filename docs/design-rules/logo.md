...
**`*hurulab`.** An asterisk, then the logotype, with no space between them.

| Part | Rule |
|---|---|
| The asterisk | `--color-text-brand`, which is `--purple-600` |
| The logotype | The heading face from [type.md](type.md), weight 700, `--color-base-black` |
...
**Slightly raised and larger than the logotype, tucked against the h.** It is set about a
...
```css
.mark i {
  font-size: 1.3em;       /* larger than the logotype */
  line-height: 0;
...
**Owner decision, 4 August 2026: the typeface's asterisk is the logo.** The earlier requirement

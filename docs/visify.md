...
distilled from was read line by line on 4 September 2026 and then deleted. This file moved
into `docs/` on 7 September 2026 when `knowledge/` was deleted. This file is what survives the read: the mechanisms that
...
here on a hurulab page is drift. What transfers is structure and behavior, never a number and
never a color.

The site is an Australian accounting firm's. Electric blue and acid lime, zero corner radius,
graph-paper and cardboard textures, torn-paper section joins, gray photo cut-outs with green
...
time. The visitor learns one structure once and then recognizes it everywhere. Three ideas do
...
Every section on their page is the same four parts in the same order:

1. ~~A small label, to say where you are~~ **BANNED HERE. Never do this.** `copy.md` section 1
   forbids a small label above a heading in any form, absolutely. The heading opens the section
   by itself. Owner ruling, restated 7 September 2026.
2. One large line, carrying the argument
3. A short paragraph
4. One action

**So the shape that transfers is three parts, not four**: the line, the paragraph, the action.

Then the field alternates, dark to light to dark, full bleed, all the way down. The rhythm of
the page comes from the color change, not from dividers or decoration. `layout.md`
section 4 already sets an alternating rhythm for tiles, and section 2 already sets full bleed,
so that part agrees with what we have.
...
tracking, color, and a switch of typeface for accent words.
...
Short, 0.2 to 0.3 seconds. What moves is color, opacity and position, and the distances are
...
- **A `1366px` max content width.** `layout.md` section 2 is full bleed with no centered
...
- **A small label above a heading**, which their section shape opens with. `copy.md` section 1
  bans it absolutely and it is struck out in section 3 above. **This is the single most
  dangerous thing in this file**, because the ban was once unwritten and survived eight
  rebuilds of a page.
...
- Their accent color is used as a fill behind text, never as text on white, because it is too
